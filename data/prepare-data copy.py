...
ROOT_DIR = './training_data'  # Ensure this path is correct and points to your training data

# Initialize the tokenizer
tokenizer = AutoTokenizer.from_pretrained('mlx-community/Mistral-7B-Instruct-v0.2-4bit')  # Replace with your tokenizer
MAX_SEQUENCE_LENGTH = 2048  # Maximum tokens per 'messages' entry
...
def generate_examples(code_snippets, unit_test_snippets, component_names):
    """
    Generates a list of examples with 'messages' field.
    Each 'messages' consists of a 'user' message with instructions and component code,
    and an 'assistant' message with the unit test code.
    ...
    for code_snippet, unit_test_snippet, component_name in zip(code_snippets, unit_test_snippets, component_names):
        ...
        # Construct the assistant message with proper formatting
        assistant_content = f"```javascript\n{unit_test_snippet}\n```"

        # Calculate token counts for both messages
        ...
        if total_tokens > MAX_SEQUENCE_LENGTH:
            print(f"Skipping `{component_name}` as it exceeds the maximum token limit ({total_tokens} tokens).")
            continue  # Skip examples that exceed the token limit

        # Append the example with 'messages' field
        examples.append({
            "messages": [
                {
                    "role": "user",
...
def save_dataset(split_name, data):
    """
    Saves the dataset split to a JSON Lines file with 'messages' field.
    """
...
if __name__ == "__main__":
    ...
    # Generate examples with 'messages' field
